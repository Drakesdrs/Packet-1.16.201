...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	static auto hudModuleTwo = moduleMgr->getModule<HudModuleTwo>();
	static auto hiveconfig = moduleMgr->getModule<HiveConfig>();
	static auto blinkMod = moduleMgr->getModule<Blink>();
	static auto testModule = moduleMgr->getModule<TestModule>();
	//static auto cubecraftconfig = moduleMgr->getModule<CubecraftConfig>();
	...
	{
		Utils::ApplyRainbow(rcolors, 0.0005);  // Increase Hue of rainbow color array
	...
	
	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			shouldRenderTabGui = hudModuleTwo->tabgui && hudModuleTwo->isEnabled();
			shouldRenderArrayList = hudModule->arraylist && hudModule->isEnabled();
			shouldRenderWatermark = hudModuleTwo->watermark && hudModuleTwo->isEnabled();
...

			{
				...
				if (shouldRenderWatermark) {
					...
					static std::string name = "SkidX v3";
#ifdef _DEBUG
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					// Fill modContainerList with Modules
					{
						...
						for (auto it : *moduleList) {
							if (it.get() != hudModule)
							if (it.get() != hudModuleTwo)
							if (it.get() != hiveconfig)
							//if (it.get() != blinkMod)
							if (it.get() != testModule)
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						auto ArrayList = moduleMgr->getModule<HudModule>();
						DrawUtils::fillRectangle(rectPos, MC_Color(0, 0, 0), ArrayList->bgOp);  //ArrayList background
						if (hudModule->arraylistOutline)
							if (hudModuleTwo->rainbow) {
								DrawUtils::fillRectangle(leftRect, MC_Color(currColor), 1.f);   //Line around modules
								DrawUtils::fillRectangle(underline, MC_Color(currColor), 1.f);  //underline
							} else {
								DrawUtils::fillRectangle(leftRect, MC_Color(255, 255, 255), 1.f);   //Line around modules
								DrawUtils::fillRectangle(underline, MC_Color(255, 255, 255), 1.f);  //underline
							}
						...
						if (!GameData::canUseMoveKeys() && rectPos.contains(&mousePos) && hudModule->clickToggle) {
							...
							if (hudModuleTwo->rainbow)
								DrawUtils::drawText(textPos, &textStr, MC_Color(currColor), textSize);  //Text color
							else 
								DrawUtils::drawText(textPos, &textStr, MC_Color(255, 255, 255), textSize);  //Text color
	...
	// Draw Info / Alert Boxes
	{
		...
		if (box) {
			...
			DrawUtils::drawRectangle(rectPos, MC_Color(255, 255, 255), 0.2f);
			box->fadeVal, 3.f;
